/**
 * ชุดอะไหล่ซ่อมบำรุง (05.10) — รวมวัสดุสิ้นเปลืองหลายรายการเป็นหนึ่งชุด ตั้งราคาขายชุดเดียว
 *
...
export interface KitInput { id?: string | null; code: string; name: string; price: number; priceB: number; priceC: number; note: string; items: KitItemInput[] }
...
export async function nextKitCode(): Promise<string> {
  return query(async (c) => {
    const { rows } = await c.query(`select count(*)::int as n from kits`);
    return `KIT-${String((rows[0]?.n ?? 0) + 1).padStart(3, '0')}`;
  });
}
...
export async function saveKit(input: KitInput): Promise<{ id: string }> {
  return mutate('stock', async (c) => {
    ...
    if (id) {
      await c.query(`update kits set code=$2, name=$3, price=$4, price_b=$5, price_c=$6, note=$7, updated_at=now() where id=$1`,
        [id, input.code, input.name, money(input.price), money(input.priceB), money(input.priceC), input.note]);
      ...
      const { rows } = await c.query(
        ...
        [input.code, input.name, money(input.price), money(input.priceB), money(input.priceC), input.note]);
    ...
    for (const it of input.items) {
      ...
      await c.query(
        ...
        [id, it.productId, it.name, it.unit, it.qty, money(it.unitCost), i++]);
    }
    return { id };
  }, { sub: 'product' });
}
...
export async function deactivateKit(id: string): Promise<void> {
  return mutate('stock', async (c) => { await c.query(`update kits set active = false, updated_at = now() where id = $1`, [id]); }, { sub: 'product' });
}
