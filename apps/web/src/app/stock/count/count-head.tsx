...
export function CountHead({ count }: { count: StockCount }) {
  ...
  /* วันที่ส่งมาตรงได้ — เลือกจากปฏิทินแล้วบันทึกทันที state ยังไม่อัปเดตในรอบเดียวกัน */
  const save = (d: string = date) => start(async () => {
    const r = await saveCountHeadAction(count.id, d, note);
      ...
      <div className="body">
        ...
        <div className="row-fields f4">
          <div className="field">
            ...
            <input className="in mono" id="ctNo" value={count.no} disabled autoComplete="off" />
          </div>
          <div className="field">
            <label htmlFor="ctDate">วันที่ตรวจนับ</label>
            <ThaiDateField id="ctDate" value={date} disabled={count.applied} full
                           onChange={setDate} onCommit={(d) => save(d)} />
          </div>
          <div className="field" style={{ gridColumn: 'span 2' }}>
            <label htmlFor="ctNote">หมายเหตุ</label>
            <input className="in" id="ctNote" value={note} disabled={count.applied} autoComplete="off"
