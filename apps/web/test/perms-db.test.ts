...
describe('ทุกเส้นทางที่ส่งข้อมูลออกต้องกันไว้ครบ', () => {
    ...
    for (const p of routes) {
      ...
      /* ยกเว้นได้เฉพาะที่ไม่ได้ส่งข้อมูลของอู่ออกไปเลย — เพิ่มเข้ารายการนี้
         ต้องมีเหตุผลกำกับเสมอ ไม่ใช่เพิ่มเพื่อให้เทสต์เขียว
           ...
           pics     ส่งรูปทีละรูปของสินค้าที่ผู้ใช้เห็นอยู่แล้ว ไม่ใช่การดึงทั้งชุด
  ...
  /**
   * route ที่ยกเว้นจากกฎข้างบนต้องกันด้วยอย่างอื่นจริง ๆ ไม่ใช่เปิดโล่ง
   * ยกเว้น healthz กับ logout ที่ตั้งใจให้เรียกได้โดยไม่ล็อกอิน
   */
  it('route ที่ยกเว้นแล้วยังส่งข้อมูลของอู่ ต้องกันด้วยสิทธิ์ดูอยู่ดี', () => {
    const src = readFileSync(join(APP, '/pics/[productId]/[sha]/route.ts'), 'utf8');
    expect(src, 'รูปเป็นข้อมูลของอู่ ต้องล็อกอินและมีสิทธิ์ดูทะเบียนสินค้าก่อน')
      .toContain("requireTab('stock', 'list')");
  });
