...
describe('หกช่องของแถบล่าง (มือถือ/แท็บเล็ต)', () => {
  it('ห้าช่องแรกเป็นเมนูที่มีอยู่จริงในผัง', () => {
    for (const k of TAB_KEYS) {
      expect(MENU.some((m) => m.key === k), `ไม่มีเมนู ${k} ในผัง`).toBe(true);
    }
  });

  /* ช่องที่หกคือ "เพิ่มเติม" ที่เปิดลิ้นชัก — ห้าช่องแรกจึงต้องไม่เกินห้า
     ไม่งั้นแถบจะแน่นจนกดพลาด (หกช่องบนจอ 375 = ช่องละ 62px ซึ่งเป็นขั้นต่ำแล้ว)
     ลำดับตามต้นแบบของทีม 16 ก.ย. 2569 — เรียงผิดก็คือคนละแถบกับที่ผู้ใช้อนุมัติ */
  it('เรียงตามต้นแบบ หน้าแรก · ลูกค้า · รายรับ · สินค้า · รายจ่าย', () => {
    expect([...TAB_KEYS]).toEqual(['home', 'customer', 'income', 'stock', 'expense']);
  });
