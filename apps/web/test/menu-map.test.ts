...
describe('ผังเมนูเทียบกับรุ่น 6.4', () => {
  ...
  it('แท็บที่ยังไม่ได้ทำถูกทำเครื่องหมายไว้ ไม่ใช่ลิงก์ตายเงียบ ๆ', () => {
    const todo = MENU.flatMap((m) => (m.subs ?? []).filter((s) => s.todo).map((s) => s.no));
    /* เคลมลูกค้า · เคลมผู้ขาย · ตรวจนับ — ช่วงที่ 4 และ 5 (ใบวางบิลทำเสร็จแล้ว) */
    expect(todo.sort()).toEqual(['05.3', '05.4', '05.5']);
  });
});
