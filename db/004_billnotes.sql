...
do $grant$ begin
  if exists (select 1 from pg_roles where rolname = 'dgl_app') then
    execute 'grant select, insert, update, delete on billnotes, billnote_docs, billnote_sequences to dgl_app';
    execute 'grant execute on function next_billnote_no(uuid, text) to dgl_app';
  end if;
end $grant$;
