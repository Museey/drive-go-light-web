...
## เทสต์ที่ต้องเปิดเบราว์เซอร์

เรื่องเมนูบังกันและเป้ากดบนจอสัมผัส ตรวจด้วยการอ่านไฟล์ CSS ไม่ได้ —
ต้องวัดกรอบจริงบนเบราว์เซอร์ ชุดนี้รันสามขนาดจอ (375 · 768 · 1280) ด้วยสเปกชุดเดียวกัน

```bash
tools/dev-seed.sh                       # ชุดนี้ต้องมีข้อมูลตัวอย่างในฐาน
DATABASE_URL=postgresql://postgres:x@localhost:5433/dgl npm run e2e -w @drivegolight/web
```

ใช้เบราว์เซอร์ที่ติดตั้งอยู่ในเครื่อง (ค่าตั้งต้น Edge Dev) เพราะเครื่องนี้ต่อ CDN
ของ Playwright ไม่ได้ เปลี่ยนได้ด้วย `PW_CHANNEL=chrome` หรือถ้าเน็ตดีแล้ว
`npx playwright install chromium` แล้วตั้ง `PW_CHANNEL=` ให้ว่าง

**ชุดนี้ไม่ได้อยู่ใน `npm test`** เพราะต้องมีเบราว์เซอร์กับเซิร์ฟเวอร์ที่รันอยู่
รันแยกตอนแก้เรื่องหน้าตาและก่อน deploy

## ตรวจอัตโนมัติตอนเปิด PR

`.github/workflows/ci.yml` รันสามอย่างนี้ให้ทุกครั้งที่เปิด PR และทุกครั้งที่ลงบน `main`

| ขั้น | คำสั่ง |
| --- | --- |
| ตรวจชนิดข้อมูล | `npm run typecheck -w @drivegolight/web` |
| เทสต์ทั้งโปรเจกต์ | `npm run test:setup` แล้ว `npm test` |
| สร้างเว็บ | `npm run build -w @drivegolight/web` |

ยก Postgres 16 ขึ้นมาเป็น service ให้ด้วย ไม่งั้นเทสต์ที่ต้องใช้ฐานข้อมูลจะ
**ข้ามทั้งชุดอย่างเงียบ ๆ** แล้วอ่านผลแล้วเหมือนเขียว ทั้งที่ไม่ได้ตรวจอะไรเลย

ชุดที่ต้องเปิดเบราว์เซอร์ยังไม่ได้อยู่ใน CI — รันเองตามหัวข้อข้างบน
...
ตรวจทุกข้อที่เครื่องตรวจแทนคนได้ — https · `/healthz` · หน้าที่ต้องปิดไว้ ·
เขตเวลา · ไมเกรชัน · สิทธิ์ของ role แอป · RLS ทุกตาราง · เจ้าของฟังก์ชัน ·
`security_invoker` ของวิว · ฟังก์ชันสร้างเจ้าของอู่ใช้ได้จริง
ตัวสคริปต์บอกเองว่าผ่านกี่ข้อ (ไม่เขียนจำนวนไว้ตรงนี้ เพราะเลขในเอกสารจะค้างวันที่เพิ่มข้อใหม่)
