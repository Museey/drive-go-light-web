...
async function checkWeb() {
  ...
  /**
   * 3.1 `/` ตอนยังไม่ล็อกอิน ต้องเป็นหน้าแนะนำระบบ ไม่ใช่หน้าแรกของอู่
   *
   * เส้นทางนี้ทำสองอย่าง (ดู app/page.tsx) จึงตรวจด้วยสถานะอย่างเดียวไม่พอ —
   * 200 อาจแปลว่าหน้าแนะนำระบบทำงานถูก หรือแปลว่าหน้าแรกของอู่หลุดออกมาให้คนนอกเห็นก็ได้
   * ต้องอ่านเนื้อหาจริงว่าได้หน้าไหนมา ทั้งสองทางพร้อมกัน
   *
   * `lp` คือกล่องนอกสุดของหน้าแนะนำระบบ · `rail` คือแถบเมนูของระบบซึ่งมีชื่ออู่อยู่บนนั้น
   */
  try {
    const res = await fetch(appUrl + '/', { redirect: 'manual' });
    const html = res.status === 200 ? await res.text() : '';
    const isLanding = /class="lp[\s"]/.test(html);
    const hasShell = html.includes('class="rail"');
    check('`/` ตอนยังไม่ล็อกอินเป็นหน้าแนะนำระบบ ไม่ใช่หน้าแรกของอู่',
      res.status === 200 && isLanding && !hasShell,
      res.status !== 200 ? `ได้ ${res.status} ไม่ใช่ 200`
        : hasShell ? 'หลุดหน้าแรกของอู่ออกมาให้คนนอกเห็น'
        : isLanding ? 'ได้หน้าแนะนำระบบ' : 'ได้ 200 แต่ไม่ใช่หน้าแนะนำระบบ');
  } catch (err) {
    check('`/` ตอนยังไม่ล็อกอินเป็นหน้าแนะนำระบบ ไม่ใช่หน้าแรกของอู่', false, String(err).slice(0, 60));
  }
   ...
   *
   * `/` ไม่อยู่ในรายการนี้แล้ว เพราะตอนยังไม่ล็อกอินมันเป็นหน้าแนะนำระบบที่ตั้งใจให้เปิดได้
   * ตัวที่ตรวจว่าไม่มีข้อมูลอู่หลุดออกมาทางนั้นคือข้อ 3.1 ข้างบน ซึ่งอ่านเนื้อหาจริง
   */
  const GUARDED = [
    ['/stock', 'ทะเบียนสินค้า'],
