...
export const FRESH_FILES = new Set([
  ...
  '013_ops_grants.sql',
  /*
   * 031 อยู่ในชุดติดตั้งใหม่ด้วย **เพราะภาพรวมของสคีมา ops อยู่ในไฟล์ที่แก้ไม่ได้แล้ว**
   *
   * 001/002 เป็นไฟล์ภาพรวม แก้ได้ทุกครั้งที่มีไมเกรชันใหม่ (SNAPSHOT_FILES)
   * แต่ฟังก์ชันของคอนโซลอยู่ใน 008/011/013 ซึ่งขึ้นเครื่องจริงไปแล้ว — แก้แล้ว deploy ล้มทันที
   * ไฟล์แก้ฟังก์ชันจึงต้องถูกรันทั้งสองทาง: ฐานเดิมได้จากการอัปเกรดตามปกติ
   * ฐานใหม่ได้จากรายการนี้ ไม่งั้นติดตั้งใหม่แล้วได้ฟังก์ชันรุ่นเก่ากลับมา
   * (เทสต์ migrations.test.ts เทียบสองทางนี้ว่าต้องได้ฟังก์ชันเหมือนกัน)
   */
  '031_ops_owner_scope.sql',
  /* 035 ด้วยเหตุผลเดียวกับ 031 — มันแก้ ops.errors และสร้าง ops.list_errors ใหม่
     ซึ่งนิยามเดิมอยู่ใน 008/011 ที่แก้ไม่ได้แล้ว · ต้องอยู่ท้ายสุดเพราะทับของจาก 011 */
  '035_error_dedup.sql',
]);
