...
export function messyBackup(): any {
  return emptyBackup({
    ...
    products: [
      ...
      product('6', { code: 'CI-001', qty: 0 }),
      product('7', { code: 'ci-001', qty: 0 }),    // ต่างแค่ตัวพิมพ์ — ถือเป็นรหัสเดียวกัน (034)
      product('8', { code: 'COL-1', qty: 0 }),
      product('9', { code: 'COL-1', qty: 0 }),     // ซ้ำ → ระบบจะเปลี่ยนเป็น COL-1-2
      product('10', { code: 'COL-1-2', qty: 0 }),  // รหัสจริงที่หน้าตาเหมือนรหัสที่ระบบเปลี่ยนให้ — ต้องไม่ชนกัน
    ],
    customers: [
