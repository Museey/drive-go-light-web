...
export interface ImportOptions {
   ...
   *
   * มีไว้เพื่อไม่ให้เตือนว่า "ยังไม่มีบัญชีเจ้าของ" ทั้งที่กำลังจะสร้างให้อยู่แล้ว —
...
export async function importBackup(
  ...
  try {
    ...
    if (restoring) {
      ...
      if (!options.ownerFollows) {
        warnings.push(
          'ยังไม่มีบัญชีเจ้าของกิจการ — สร้างด้วย --owner-email= แล้วส่งลิงก์ตั้งรหัสผ่านให้เจ้าของอู่',
        );
      }
    }
